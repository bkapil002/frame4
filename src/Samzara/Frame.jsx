            ...
            <div className="flex flex-col lg:flex-row flex-1 p-4 gap-4">
              ...
              <div className="lg:w-3/5 w-full flex flex-col items-center bg-white shadow rounded-lg p-4">
                ...
                <div className="flex gap-2 mt-4 flex-wrap justify-center">
                  {isAdmin && (
                    <button onClick={() => setCamera(a => !a)} className="bg-blue-900 text-white px-3 py-1 text-sm rounded-full">
                 ...
                 {!isAdmin && (
                  <button onClick={() => setCamera(a => !a)} className="bg-blue-900 text-white px-3 py-1 text-sm rounded-full">
                    {cameraOn ? "Request to Share" : "Turn Cam Off"}
                  </button>
                 )} 
                  <button onClick={() => setCalling(false)} className="bg-blue-900 text-white px-3 py-1 text-sm rounded-full">
                    End call
                  </button>
                 

                  <button onClick={() => setMic(a => !a)} className="bg-blue-900 text-white px-3 py-1 text-sm rounded-full">
      ...
      <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {!isAdmin && (
          <div key={email} className="text-center">
            ...
            />
            <div className="text-sm font-medium mt-2 text-gray-800">{names[email] || user.name || "You"}</div>
          </div>
        )}
